...
PALABRA1_OK="6d6f7149"  #-- Mensaje cifrado
PALABRA2_OK="0000000a"  #-- Salto de linea (no cifrado)

##-- Salida exacta por la consola:
OUTPUT_EXACT="Introduce cadena: Cadena cifrada: Iqom"
...
####-- Debe contener el mensaje cifrado: "Iqom" en hexa: 6d6f7149
cifrado1=`head -n1 $MEMFILE | tail -n1`

##-- Comprobar el mensaje cifrado (primera palabra de memoria)
if [ "$cifrado1" = $PALABRA1_OK ]
then
  echo "> ✅️ Primera palabra de memoria: $cifrado1"
else
...
if [ "$ciclos" -lt 53 ]
then
   echo "> ✅️🎖️  BONUS!!!. Menos de 53 ciclos"
fi
