...
if data["a"] == data_ok["a"]:
    print(f"> ✅️ a: {hex(data['a'])}")
else:
    print(f"> ❌️ a: {hex(data['a'])}."\
          f"Debería ser: {hex(data_ok['a'])}")
    hay_errores = True

#-- Comprobar la variable 2
if data["b"] == data_ok["b"]:
    print(f"> ✅️ b: {hex(data['b'])}")
else:
    print(f"> ❌️ b: {hex(data['b'])}."\
          f"Debería ser: {hex(data_ok['b'])}")
    hay_errores = True
...
if not hay_errores:
    ...
    if data["f"] == data_ok["f"]:
        print(f"  > ✅️ Resultado guardado en memoria")
        ok_varf = True
    else:
        print(f"  > ❌️ Valor incorrecto en 0x1001000c: {hex(data['f'])}."\
            f"Debería ser: {hex(data_ok['f'])}")
    ...
    if (ok_varf):
        ...
        if instrucciones < 11:
            print(f"  > ✅️ Menos de 11 instrucciones")
            ok_inst = True
        else:
            print(f"  > ❌️ Más de 11 instrucciones...")
